...
- M7 shades (noterat, ej byggt): (1) `battler.gdshader`s kantljus (`rim_offset` i UV) blir en tjock orange kontur på små texturer (160–380 px) och dubblar shades inbakade rim – överväg rim i pixlar i stället för UV, eller `rim_strength` per manifestpost. (2) Ögonen/glöden multipliceras med `light_tint` och slocknar nästan i korridoren; en emissiv mask (andra textur eller alfa-kanal-trick) skulle låta ögon lysa oberoende av ljuset. (3) Silhuett-takten på två rutors håll syntes inte alls i `shot_battlers.gd` (mörk figur mot mörk gång, 480×900) – kontrollera i riktigt spel. (4) Ljusriktningen: PM-briefen sa "övre vänster", ART_DIRECTION_V2 §4 och shadern säger höger; shades följer shadern (`light_from` i `shades.json` vänder på en rad).
