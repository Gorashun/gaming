...
- M7 shades (noterat, ej byggt): (1) ~~dubbel rim~~ och (2) ~~ögon som slocknar~~ löst i M7-poleringen (`baked_rim`, `emissive`). Kvar: shaderns rim i UV blir fortfarande tjock på små Pipoya-/Ækashics-texturer – rim i pixlar vore rättare. Glödlagret påverkas av miljöns dimma (unshaded-materialet dimmas av Godot); märks inte på stridsavstånd. (3) Silhuett-takten på två rutors håll syntes inte alls i `shot_battlers.gd` (mörk figur mot mörk gång, 480×900) – kontrollera i riktigt spel. (4) Ljusriktningen: PM-briefen sa "övre vänster", ART_DIRECTION_V2 §4 och shadern säger höger; shades följer shadern (`light_from` i `shades.json` vänder på en rad).
