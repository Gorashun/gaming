...
- **Polering (PM: shades skeppas som interim).** Manifestets `baked_rim` stänger av shaderns kantljus för shades (ingen dubbel orange kontur; Pipoya/Ækashics oförändrade). Nytt glödlager: `render_shades.js` skriver `<ID>_emissive.png`, `normalize_art.py` beskär det som bilden (`emissive_src` → manifestets `emissive`), `battler.gdshader` adderar det efter `light_tint` med puls 0,8–1,0 på 2 s (ingen puls vid reducerad rörelse, dolt av silhuetten). SLAGJAW är en komposit (daemon-skull framför muscular-torso och två bat-wing, 1024×640) som fyller bosshöjden 3,0 m. SLAG_MOTH mindre (1,57×1,17 m) så den inte läser som den bevingade bossen. `tests/test_battler_glow.gd` (5), `test_normalize_art.py` +1.
...
- **Steg 4, character sheet v2.** Paperdollen är ersatt av det målade porträttet (`hero.portrait.a/b`) i ett varmt sken; de sju slotsen bär föremålets Ravenmore-ikon (`gear.<ID>`) i sin raritetsram (`rarity.frame.<namn>`), en tom slot visar sin egen ikon nedtonad och en låst slot "LV n". Gear läses via `Hero.equipped(slot)`, `Item.icon_id`, `Item.rarity` när controllern skickar `"hero"`; annars relikerna som i M5. CHANGE LOOK byter porträtt (och `Hero.body_variant`) och emitterar `look_changed`. Integrationspunkterna står i `docs/M6_A_NOTES.md`. `tests/test_art_sheet.gd` (8).
- **Steg 5, credits.** `src/game/credits/credits_screen.tscn` renderar `assets/credits.json` (en rubrik per sektion, varje källas `attribution_lines` ordagrant, källor utan känd sektion tappas aldrig) plus fonternas OFL-notiser (copyrightraden ordagrant ur varje `OFL-*.txt`, testat) och den fulla OFL-texten när filen finns. Nås från inställningarna via en ny rad. `tests/test_art_credits.gd` (5).
- **Steg 6, verifiering (HEAD aa5c02e, isolerad worktree utan andra agenters ocommittade filer).** `--import` utan skriptfel; gdUnit4 542/542 gröna (35 sviter); rökprov en + sv `SMOKE OK (29 screenshots)`, korridor 15,4 s/run (budget 90 s), tysta steg 3 (max 3), bildtid under kedja p50 63,8 ms / p95 78,7 ms på xvfb/llvmpipe (sessionens baslinje före stilskiktet: p50 63,1 / p95 92,6). Webbygge via `tools/web/build_web.sh`; `manifest.json`, `credits.json` och konsten ligger i pck:n, `OFL-*.txt` gör det inte (export-filtret). Playwright 480×900 i `docs/screenshots/m6a/` (`?start=corridor` in i strid med skadesiffra, `?start=sheet`, `?start=town`) plus mobilrenderarens skärmdumpar i `m6a/mobile/`.
